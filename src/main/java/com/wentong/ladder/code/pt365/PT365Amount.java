...
@AllArgsConstructor
@MappedClass
public class PT365Amount {

    @MappedField(expression = "temporaryModel.storeChannelBean.special_key", desc = "扣款的365账号(该账户需要第三方平台验证通过)(需要和下单接口的user365Id参数值一致，不传使用三方平台主账号进行计算运费)", type = MappedType.EXPRESSION)
    private String user365Id;
    @MappedField(expression = "temporaryModel.storeChannelBean.transportLongitude", desc = "发货地坐标经度", type = MappedType.EXPRESSION)
